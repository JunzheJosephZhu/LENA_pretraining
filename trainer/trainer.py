...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (mixture, clean, name) in enumerate(pbar):
            # clean is just mixture in identity mapping
            mixture = mixture.to(self.device)
            clean = clean.to(self.device)

            self.optimizer.zero_grad()
            enhanced, encoding = self.model(mixture)
            mse = self.loss_function(clean, enhanced)
            l1 = self.lamb * encoding.abs().mean()
            loss = mse + l1
            loss.backward()
            self.optimizer.step()
            ...
            loss_total += mse.item()
            l1_total += l1.item()
            pbar.set_description("epoch %d iteration %d: MSE=%f L1=%f" % (epoch, i, loss_total / (i + 1), l1_total / (i + 1)))
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, clean, name) in enumerate(tqdm(self.validation_data_loader)):
            try:
        ...
        score = get_metrics_ave(stoi_c_e)
        return score
